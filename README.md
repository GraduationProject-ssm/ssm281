...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1gn8XeNE2J?p=78)
...
随着计算机技术的发展以及计算机网络的逐渐普及，互联网成为人们查找信息的重要场所，二十一世纪是信息的时代，所以信息的交换和信息流通显得特别重要。因此，开发合适的基于SSM框架的电脑测评系统成为用户必然要走的一步棋。开发合适的基于SSM框架的电脑测评系统，可以方便管理人员对基于SSM框架的电脑测评系统的管理，提高信息管理工作效率及查询效率，有利于更好的为用户提供服务。

1.2研究目的

随着互联网技术的快速发展，网络时代的到来，网络信息也将会改变当今社会。各行各业在日常用户经营管理等方面也在慢慢的向规范化和网络化趋势汇合。基于SSM框架的电脑测评系统的信息化程度体现在将互联网与信息技术应用于经营与管理，以现代化工具代替传统手工作业。无疑，使用网络信息化管理使信息管理更先进、更高效、更科学，信息交流更迅速。

采用电子化的存储方式就会带来很大的改善，而且给用户的查询带来了很大便利，因此设计一个基于SSM框架的电脑测评系统刻不容缓，能够提高用户在信息技术方面的展示水平。
## 1.3系统设计思想
...
## 2.3 SSM框架简介
SSM框架，是Spring+Spring MVC+MyBatis的缩写，这个是继SSH之后，目前比较主流的Java EE用户级框架，适用于搭建各种大型的用户级应用系统。

1.Spring简介

Spring是一个开源框架，Spring是于2003年兴起的一个轻量级的Java开发框架，由Rod Johnson在其著作Expert One-On-One J2EE Development and Design中阐述的部分理念和原型衍生而来。它是为了解决用户应用开发的复杂性而创建的。Spring使用基本的JavaBean来完成以前只可能由EJB完成的事情。然而，Spring的用途不仅限于服务器端的开发。从简单性、可测试性和松耦合的角度而言，任何Java应用都可以从Spring中受益。简单来说，Spring是一个轻量级的控制反转（IoC）和面向切面（AOP）的容器框架。
...
# 3系统分析
## 3.1可行性分析
通过对本基于SSM框架的电脑测评系统实行的目的初步调查和分析，提出可行性方案并对其一一进行论证。我们在这里主要从技术可行性、经济可行性、操作可行性等方面进行分析。
### 3.1.1技术可行性
本基于SSM框架的电脑测评系统采用JAVA作为开发语言，SSM框架，是基于WEB平台的B/S架构系统。
...
因此，基于SSM框架的电脑测评系统在开发技术上具有很高可行性，且开发人员掌握了一定的开发技术，所以此系统的开发技术具有可行性。
### 3.1.2经济可行性
本基于SSM框架的电脑测评系统采用的软件都是开源的，这样能够削减很多的精力和资源，降低开发成本。同时对计算机的配置要求也极低，即使是淘汰下来的计算机也能够满足需要，因此，本系统在经济上是完全具有可行性的，所以在经济上是十分可行的。
### 3.1.3操作可行性
本基于SSM框架的电脑测评系统的界面简单易操作，用户只要平时有在用过电脑，都能进行访问和操作。本系统具有易操作、易管理、交互性好的特点，在操作上是非常简单的，因此在操作上具有很高的可行性。

综上所述，此系统开发目标已明确，在技术、经济和操作方面都具有很高的可行性，并且投入少、功能完善、管理方便，因此系统的开发是完全可行的。
## 3.2系统性能分析
### 3.2.1 系统安全性
此基于SSM框架的电脑测评系统要严格控制管理权限，具体要求如下：

（1）要想对基于SSM框架的电脑测评系统进行管理，首先要依靠用户名和密码在系统中登陆，无权限的用户不可以通过任何方式登录系统和对系统的任何信息和数据进行查看，这样可以保证系统的安全可靠性和准确性。
...
图4.14电脑分类实体属性图
### 4.3.2数据库设计表
基于SSM框架的电脑测评系统需要后台数据库，下面介绍数据库中的各个表的详细信息：
...
|fenleimingcheng|varchar(200)|是|NULL|分类名称|
...
|id (主键)|bigint(20)|否||主键|
|addtime|timestamp|否|CURRENT\_TIMESTAMP|创建时间|
|leixingmingcheng|varchar(200)|是|NULL|类型名称|
|shuliang|int(11)|否||数量|
表4.3电脑评测
...
|diannaofenlei|varchar(200)|是|NULL|电脑分类|
|diannaopinpai|varchar(200)|是|NULL|电脑品牌|
|diannaofengmian|varchar(200)|是|NULL|电脑封面|
|cepingxinxi|longtext|是|NULL|测评信息|
...
|id (主键)|bigint(20)|否||主键|
|addtime|timestamp|否|CURRENT\_TIMESTAMP|创建时间|
|pinpaibianhao|varchar(200)|否||品牌编号|
|pinpaimingcheng|varchar(200)|是|NULL|品牌名称|
|pinpaifengmian|varchar(200)|是|NULL|品牌封面|
|xiangguandiannao|longtext|是|NULL|相关电脑|
...
|id (主键)|bigint(20)|否||主键|
|addtime|timestamp|否|CURRENT\_TIMESTAMP|创建时间|
|diannaobianhao|varchar(200)|否||电脑编号|
|diannaomingcheng|varchar(200)|是|NULL|电脑名称|
|diannaofenlei|varchar(200)|是|NULL|电脑分类|
|diannaopinpai|varchar(200)|是|NULL|电脑品牌|
|diannaofengmian|varchar(200)|是|NULL|电脑封面|
|diannaojiage|int(11)|否||电脑价格|
|diannaopeizhi|longtext|是|NULL|电脑配置|
|thumbsupnum|int(11)|是|0|赞|
|crazilynum|int(11)|是|0|踩|
|clicktime|datetime|是|NULL|最近点击时间|
